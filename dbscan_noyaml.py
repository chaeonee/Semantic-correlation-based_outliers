...
#model_path = os.path.abspath("./wordmodels/coco15000_model_10.bin")
model = g.KeyedVectors.load_word2vec_format("wordmodels/coco15000_model_10.bin", binary=True)
...
def word_distance(w1,w2):
    ...
    if word1 in model.vocab:
        if word2 in model.vocab: #두 단어 모두 voca(cocodata)에 있는 단어인지 확인
            dis = 1-model.wv.similarity(w1=word1,w2=word2)
        else:
            dis = 2
    else: # 없으면 distance를 가장 크게 설정
        dis = 2
...
def get_outliers(classlist):
    ...
    outliers = []
    if len(cluster) > 1:
        if -1 in clusters: #-1이면 outlier
            for i in range(len(clusters)):
                if -1 == clusters[i]:
                    outliers.append(i)
        else: #-1이 없을 때는 가장 작은 크기의 cluster가 outlier
